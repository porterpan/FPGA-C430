...
void make_udp_reply_from_request(uint8_t *buf,char *data,uint8_t datalen,uint16_t port)
{
        ...
        enc28j60_packet_send(UDP_HEADER_LEN+IP_HEADER_LEN+ETH_HEADER_LEN+datalen,buf);
}
...
void make_tcp_synack_from_syn(uint8_t *buf)
{
        ...
        enc28j60_packet_send(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN,buf);
}
...
void make_tcp_ack_from_any(uint8_t *buf)
{
        ...
        enc28j60_packet_send(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN,buf);
}
...
void make_tcp_ack_with_data(uint8_t *buf,uint16_t dlen)
{
        ...
        buf[TCP_CHECKSUM_H_P]=j>>8;
        buf[TCP_CHECKSUM_L_P]=j& 0xff;
        enc28j60_packet_send(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN,buf);
}
